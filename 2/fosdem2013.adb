...
procedure Fosdem2013 is
   ...
   procedure Draw is new Mandelbrots.Compute_G (Draw);
         ...
         case E.Response_Type is
            when XCB.Event.Configure_Notify =>
               ...
               Mandelbrot_Set := new Mandelbrots.Set_T'
                 (Mandelbrots.Create
                    (Image_Rectangle => (E.X, E.Y, E.Width, E.Height),
                     Bottom_Left     => (-2.5, -1.0),
                     Top_Right       => (1.0,   1.0),
                     Max_Iterations  => 10_000));
            when XCB.Event.Expose =>
               if Mandelbrot_Set = null then
                  Ada.Text_IO.Put_Line ("Expose before Configure_Notify");
               else
