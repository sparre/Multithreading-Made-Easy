package body Mandelbrots is
   ...
   procedure Compute_G (Set              : in Set_T;
      ...
      task type Rectangle_Worker is
         entry Set_Rectangle (R : in XCB.Rectangle.T);
      end Rectangle_Worker;

      task body Rectangle_Worker is
         Rectangle : XCB.Rectangle.T;
      begin
         accept Set_Rectangle (R : in XCB.Rectangle.T) do
            Rectangle := R;
         end Set_Rectangle;
         Process (Rectangle);
      end Rectangle_Worker;

      Workers : array (1 .. Sub_Rectangles_X, 1 .. Sub_Rectangles_Y) of
        Rectangle_Worker;
